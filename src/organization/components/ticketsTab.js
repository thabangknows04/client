...
const TicketsTab = ({ eventData, setEventData, formatDate, onEventUpdated }) => {
  ...
  const handleEdit = (ticket) => {
    ...
    setNewTicket({ ...ticket, availableUntil: formatDate(ticket.availableUntil, 'yyyy-MM-ddTHH:mm') }); // Format for input type="datetime-local"
    setShowTicketForm(true);
  };
  ...
  const handleDelete = async (ticketId) => {
    if (window.confirm("Are you sure you want to delete this ticket type?")) {
      try {
        ...
        await axios.delete(`/api/events/${eventData._id}/tickets/${ticketId}`);
...
setEventData(prev => ({
        ...
        <div className="p-6">
          <div className="flex justify-between items-start gap-4">
            <div className="flex-1 min-w-0">
              <h3 className="text-xl font-semibold text-gray-900 truncate">{ticket.name}</h3>
              {ticket.description && (
  <p className="text-sm text-gray-500 mt-2">
    ...
    <div className="col-span-3 py-12 bg-white rounded-xl border-2 border-dashed border-gray-300 text-center">
      ...
      <h3 className="mt-2 text-lg font-medium text-gray-900">No ticket types added</h3>
          ...
          <div className="bg-white rounded-lg p-6 w-full max-w-md">
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <div className="flex justify-end space-x-3 pt-2">
                ...
                >
                  {editingTicket ? <><FiEdit className="w-4 h-4" /> Update Ticket</> : <><FiPlus className="w-4 h-4" /> Add Ticket</>}
                </button>
              </div>
            </form>
          </div>
